...
def preprocess_dataset(dataset, token_to_index):
    def add_input_ids_and_labels(example):
        # for ph in example['phonemes']:
            # if ph not in token_to_index:
                # print(ph)
        input_ids = [token_to_index[ph] for ph in example['phonemes'] if ph in token_to_index]
    ...
    dataset = dataset.map(add_input_ids_and_labels, remove_columns=['phonemes'], num_proc=32)
    return dataset
...
for split in splits:
    loaded_datasets[split] = preprocess_dataset(loaded_datasets[split], token_to_index)
